...
class CriticalLineAlgorithm:
    ...
    def __init__(self, weight_bounds=(0, 1), calculate_expected_returns="mean"):
        ...
        devadarsh.track('CriticalLineAlgorithm')
    ...
    def allocate(self, asset_names=None, asset_prices=None, expected_asset_returns=None, covariance_matrix=None,
        ...
        cov_matrix, expected_returns, lower_bounds, upper_bounds = self._initialise(asset_prices=asset_prices, resample_by=resample_by,
                                                                                    expected_asset_returns=expected_asset_returns,
                                                                                    covariance_matrix=covariance_matrix)

        # Compute the turning points, free sets and weights
        free_weights, weights = self._init_algo(expected_returns, lower_bounds, upper_bounds)
        ...
        while True:
            ...
            lambda_in, i_in, bi_in = self._bound_free_weight(cov_matrix, expected_returns, lower_bounds, upper_bounds, free_weights)

            # 2) Free one bounded weight
            lambda_out, i_out = self._free_bound_weight(cov_matrix, expected_returns, free_weights)

            # 3) Compute minimum variance solution
            if (lambda_in is None or lambda_in < 0) and (lambda_out is None or lambda_out < 0):
                self.lambdas.append(0)
                covar_f, covar_fb, mean_f, w_b = self._get_matrices(cov_matrix, expected_returns, free_weights)
        ...
        self._compute_solution(asset_names, solution, cov_matrix, expected_returns)
    ...
    def _initialise(self, asset_prices, expected_asset_returns, covariance_matrix, resample_by):
        ...
        expected_returns = np.array(expected_returns).reshape((len(expected_returns), 1))
        ...
        if isinstance(self.weight_bounds[0], numbers.Real):
            lower_bounds = np.ones(expected_returns.shape) * self.weight_bounds[0]
        else:
            lower_bounds = np.array(self.weight_bounds[0]).astype(float).reshape(expected_returns.shape)

        # Intialise upper bounds
        if isinstance(self.weight_bounds[1], numbers.Real):
            upper_bounds = np.ones(expected_returns.shape) * self.weight_bounds[1]
        else:
            upper_bounds = np.array(self.weight_bounds[1]).astype(float).reshape(expected_returns.shape)
    ...
    def _init_algo(expected_returns, lower_bounds, upper_bounds):
        ...
        structured_array = np.zeros((expected_returns.shape[0]), dtype=[("id", int), ("mu", float)])
        expected_returns_ = [expected_returns[i][0] for i in range(expected_returns.shape[0])]  # dump array into list

        # Fill structured array
        structured_array[:] = list(zip(list(range(expected_returns.shape[0])), expected_returns_))
        ...
        while np.sum(weights) < 1:
            ...
            weights[expected_returns[index][0]] = upper_bounds[expected_returns[index][0]]
    ...
    def _get_matrices(self, cov_matrix, expected_returns, free_weights):
        ...
        bounded_weights = self._get_bounded_weights(expected_returns, free_weights)
        covar_fb = self._reduce_matrix(cov_matrix, free_weights, bounded_weights)
    ...
    def _get_bounded_weights(self, expected_returns, free_weights):
        ...
        return self._diff_lists(list(range(expected_returns.shape[0])), free_weights)
        ...
        while True:
            ...
            if abs(sum(self.weights[index_1]) - 1) > tol:
                ...
                for index_2 in range(len(self.weights[index_1])):
                    if (
                            self.weights[index_1][index_2] - lower_bounds[index_2] < -tol
                            or self.weights[index_1][index_2] - upper_bounds[index_2] > tol
                    ):
                        flag = True
                        break
            if flag is True:
        ...
        if len(free_weights) > 1:
            ...
            for i in free_weights:
                lambda_i, b_i = self._compute_lambda(
                    covar_f_inv, covar_fb, mean_f, w_b, j, [lower_bounds[i], upper_bounds[i]]
                )
    ...
    def _free_bound_weight(self, cov_matrix, expected_returns, free_weights):
        ...
        if len(free_weights) < expected_returns.shape[0]:
            bounded_weight_indices = self._get_bounded_weights(expected_returns, free_weights)
            for i in bounded_weight_indices:
                covar_f, covar_fb, mean_f, w_b = self._get_matrices(cov_matrix, expected_returns, free_weights + [i])
                covar_f_inv = np.linalg.inv(covar_f)
    ...
    def _compute_solution(self, assets, solution, covariance_matrix, expected_returns):
        ...
        if solution == "max_sharpe":
            self.max_sharpe, self.weights = self._max_sharpe(covariance_matrix, expected_returns)
            ...
            self.efficient_frontier_means, self.efficient_frontier_sigma, self.weights = self._efficient_frontier(covariance_matrix, expected_returns)
    ...
    def _max_sharpe(self, cov_matrix, expected_returns):
        ...
        for i in range(len(self.weights) - 1):
            ...
            kwargs = {"minimum": False, "args": (cov_matrix, expected_returns, w_0, w_1)}
