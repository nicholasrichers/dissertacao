...
from portfoliolab.clustering.base import BaseClusteringOptimizer


class NestedClusteredOptimisation(BaseClusteringOptimizer):
    """
    ...
    def __init__(self):
        ...
        super().__init__()
        devadarsh.track('NestedClusteredOptimisation')
    ...
    def allocate_cvo(self, cov, mu_vec=None):
        ...
        if self.risk_estimator.is_matrix_invertible(cov):
    ...
    def allocate_nco(self, cov, mu_vec=None, max_num_clusters=None, n_init=10, asset_names: list = None):
        """
        ...
        if isinstance(cov, pd.DataFrame):
            cov = pd.DataFrame(cov.values)  # Covariance matrix with int rows/indices instead of str
        else:
        ...
        self.weights = pd.DataFrame(w_nco.T, columns=asset_names)
    ...
    def allocate_mcos(self, mu_vec, cov, num_obs, num_sims=100, kde_bwidth=0.01, min_var_portf=True,
        ...
        for simulation in range(num_sims):
            ...
            if kde_bwidth > 0:
                cov_simulation = self.risk_estimator.denoise_covariance(cov_simulation,
                                                                        num_obs / cov_simulation.shape[1],
                                                                        kde_bwidth)
            ...
            asset_names = list(range(cov_simulation.shape[0]))
            w_nco.loc[simulation] = self.allocate_nco(cov_simulation, mu_simulation,
                                                      int(cov_simulation.shape[0] / 2), asset_names=asset_names).values.T.flatten()
    ...
    def _cluster_kmeans_base(corr, max_num_clusters=None, n_init=10):
        ...
        for init in range(1, n_init + 1):
            ...
            for num_clusters in range(2, max_num_clusters + 1):
                ...
                stat_coef = (silh_coef.mean() / silh_coef.std()) \
                    if silh_coef.std() != 0 \
                    else np.inf
                stat_coef_optimal = (silh_coef_optimal.mean() / silh_coef_optimal.std()) \
                    if silh_coef_optimal.std() != 0 \
                    else np.inf

                stat = (stat_coef, stat_coef_optimal)
    ...
    def form_true_matrix(self, num_blocks, block_size, block_corr, std=None):
        ...
        cov_matrix = self.risk_estimator.corr_to_cov(corr_matrix, std)
