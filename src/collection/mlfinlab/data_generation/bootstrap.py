...
def row_bootstrap(mat, n_samples=1, size=None):
    ...
    for _ in range(n_samples):
        # Sample with replacement the number of rows indices given by the size parameter
        # Convert it to a matrix of rows indices
...
def block_bootstrap(mat, n_samples=1, size=None, block_size=None):
    ...
    # If the block size is not given, use a block size of 10% of given matrix size
    # Each dimension has a minimum size of 2
    if not block_size:
