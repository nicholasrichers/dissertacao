...
def return_over_benchmark(prices, benchmark=0, binary=False, resample_by=None, lag=True):
    ...
    :param prices: (pd.Series/pd.DataFrame) Time indexed prices to compare returns against a benchmark.
    ...
    :return: (pd.Series/pd.DataFrame) Excess returns over benchmark. If binary, the labels are -1 if the
