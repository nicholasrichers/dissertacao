...
def mean_decrease_accuracy(model, X, y, cv_gen, clustered_subsets=None, sample_weight_train=None,
    ...
    * The CV must be purged and embargoed.
    ...
    # Return aggregated mean importance over all repeats
...
def _stacked_mean_decrease_accuracy_round(model: object, X_dict: dict, y_dict: dict, cv_gen: BaseCrossValidator,
    ...
    * The CV must be purged and embargoed.
    ...
    X_columns = X_dict[first_asset].columns
    ...
    fold_metrics_values, features_metrics_values = pd.Series(dtype='float64'), pd.DataFrame(columns=X_columns)
    # Generating a numpy random state object for the given random_state
    rs_obj = np.random.RandomState(seed=random_state)
    # Clustered feature subsets will be used for CFI if clustered_subsets exists
    # else will operate on the single column as MDA
    feature_sets = clustered_subsets if clustered_subsets else [[x] for x in X_columns]
    for i, (train, test) in enumerate(cv_gen.split(X_dict=X_dict)):
        ...
        fit = model.fit(X=train_X, y=train_y, sample_weight=sample_weight_train)
        pred = fit.predict(test_X)

        # Get overall metrics value on out-of-sample fold
        if require_proba is True:
            prob = fit.predict_proba(test_X)
            fold_metrics_values.loc[i] = scoring(test_y, prob, sample_weight=sample_weight_score,
                                                 labels=model.classes_)
        else:
            fold_metrics_values.loc[i] = scoring(test_y, pred, sample_weight=sample_weight_score)

        # Get feature specific metric on out-of-sample fold
        for j in feature_sets:
            X1_ = test_X.copy(deep=True)
            for j_i in j:
                rs_obj.shuffle(
                    X1_[j_i].values)  # Permutation of a single column for MDA or through the whole subset for CFI
            if require_proba is True:
                prob = fit.predict_proba(X1_)
                features_metrics_values.loc[i, j] = scoring(test_y, prob,
                                                            sample_weight=sample_weight_score,
                                                            labels=model.classes_)
            else:
                pred = fit.predict(X1_)
                features_metrics_values.loc[i, j] = scoring(test_y, pred,
                                                            sample_weight=sample_weight_score)
...
def stacked_mean_decrease_accuracy(model: object, X_dict: dict, y_dict: dict, cv_gen: BaseCrossValidator,
    ...
    * The CV must be purged and embargoed.
    ...
    feature_importances_df = pd.DataFrame()
    for i in range(n_repeat):
        mda_round = _stacked_mean_decrease_accuracy_round(model=model, X_dict=X_dict, y_dict=y_dict, cv_gen=cv_gen,
                                                          clustered_subsets=clustered_subsets,
                                                          sample_weight_train_dict=sample_weight_train_dict,
                                                          sample_weight_score_dict=sample_weight_score_dict,
                                                          scoring=scoring, require_proba=require_proba,
                                                          random_state=i)
        feature_importances_df = feature_importances_df.append(mda_round)
    # Return aggregated mean importance over all repeats
