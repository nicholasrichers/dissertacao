...
def get_stacked_weights_by_return(triple_barrier_events_dict: dict, close_series_dict: dict, num_threads: int = 5,
    ...
    sample_weight_results_dict = {}  # Dictionary of asset: pd.Series with sample weights.
    for asset in triple_barrier_events_dict.keys():
        smpl_w = get_weights_by_return(triple_barrier_events=triple_barrier_events_dict[asset],
                                       close_series=close_series_dict[asset],
                                       num_threads=num_threads, verbose=verbose)
        sample_weight_results_dict[asset] = smpl_w

    return sample_weight_results_dict
...
def get_stacked_weights_time_decay(triple_barrier_events_dict: dict, close_series_dict: dict, decay: int = 0.5,
    ...
    sample_weight_results_dict = {}  # Dictionary of asset: pd.Series with sample weights.
    for asset in triple_barrier_events_dict.keys():
        smpl_w = get_weights_by_time_decay(triple_barrier_events=triple_barrier_events_dict[asset],
                                           close_series=close_series_dict[asset], decay=decay, num_threads=num_threads,
                                           verbose=verbose
